...
class Akka23Scala211RemoteTraceSpec extends EchoTraceSpec(RemoteTraceTest.config1) {
  ...
  "Remote tracing" must {
    ...
    "trace across nodes" in {
      ...
      val remoteAddress = "akka.tcp://Akka23Scala211RemoteTraceSpec@%s:%s/user/%s".format(Host, Port2, "b")
