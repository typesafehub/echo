...
class Akka20SamplingSpec extends AtmosTraceSpec(SamplingSpec.config) {
  ...
  "Sampling" must {
    ...
    "only trace at sampling rate" in {
      ...
      val actor1 = system.actorOf(Props[TestActor], "sampling1") // sampled = 1 (27 events, 1 trace)
      val actor2 = system.actorOf(Props[TestActor], "sampling2") // sampled = 1 (27 events, 1 trace)
