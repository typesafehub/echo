...
class Akka20TagsSpec extends AtmosTraceSpec(TagsSpec.config) {
  import TagsSpec._
  ...
  "Tags" must {
    ...
    "be attached to actor info" in {
      ...
      val actor1 = system.actorOf(Props[TestActor], "tags1") // 27 events
      val actor2 = system.actorOf(Props[TestActor], "tags2") // 27 events
