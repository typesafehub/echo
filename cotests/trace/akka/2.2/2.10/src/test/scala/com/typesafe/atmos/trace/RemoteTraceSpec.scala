...
class Akka22Scala210RemoteTraceSpec extends AtmosTraceSpec(RemoteTraceTest.config1) {
  ...
  "Remote tracing" must {
    ...
    "trace across nodes" in {
      ...
      val remoteAddress = "akka.tcp://Akka22Scala210RemoteTraceSpec@%s:%s/user/%s".format(Host, Port2, "b")
