...
class Akka22Scala211RemoteTraceSpec extends AtmosTraceSpec(RemoteTraceTest.config1) {
  ...
  "Remote tracing" must {
    ...
    "trace across nodes" in {
      ...
      val remoteAddress = "akka.tcp://Akka22Scala211RemoteTraceSpec@%s:%s/user/%s".format(Host, Port2, "b")
