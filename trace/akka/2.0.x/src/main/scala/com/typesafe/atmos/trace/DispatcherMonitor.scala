...
class DispatcherMonitor(trace: Trace) extends Actor with ActorLogging {
  ...
  val scheduled: Cancellable =
    context.system.scheduler.schedule(pollInterval, pollInterval, self, Tick)
    ...
    for (dispatcher ← dispatchers) {
      dispatcher match {
        case x: Dispatcher ⇒
          ...
          val ref = method.invoke(x).asInstanceOf[AtomicReference[_]]
          val executorService = ref.get match {
