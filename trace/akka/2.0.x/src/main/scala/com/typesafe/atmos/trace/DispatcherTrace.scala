...
class TracedRunnable(runnable: Runnable, tracer: ActorSystemTracer, context: TraceContext, info: TaskInfo) extends Runnable {
  def run: Unit = {
    tracer.trace.local.start(context)
    tracer.dispatcher.runnableStarted(info)
    try {
      runnable.run()
    } finally {
      tracer.dispatcher.runnableCompleted(info)
