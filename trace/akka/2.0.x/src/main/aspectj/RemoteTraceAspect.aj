...
privileged public aspect RemoteTraceAspect {
  ...
  before(RemoteTransport transport, RemoteSettings settings, ActorSystemImpl system, RemoteActorRefProvider provider):
    execution(akka.remote.RemoteTransport+.new(..)) &&
    this(transport) &&
    args(settings, system, provider)
  {
  ...
  before(RemoteActorRef actorRef, RemoteActorRefProvider provider, RemoteTransport remote, ActorPath path, InternalActorRef getParent):
    execution(akka.remote.RemoteActorRef.new(..)) &&
    this(actorRef) &&
    args(provider, remote, path, getParent)
  {
  ...
  returning(RemoteMessageProtocol.Builder builder):
    execution(* akka.remote.RemoteMarshallingOps$class.createRemoteMessageProtocolBuilder(..)) &&
  ...
  Object around(RemoteMarshallingOps remote, RemoteMessage remoteMessage):
    execution(* akka.remote.RemoteMarshallingOps$class.receiveMessage(..)) &&
    args(remote, remoteMessage)
  {
