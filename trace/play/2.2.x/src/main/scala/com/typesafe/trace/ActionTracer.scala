 ...
 */
abstract class ActionTracer extends DefaultTracer {
  def enabled: Boolean
  def trace: Trace
 ...
 */
class DefaultActionTracer(settings: Trace.Settings) extends ActionTracer {
...
class DisabledActionTracer(settings: Trace.Settings) extends ActionTracer {
  val trace = null
  val action = null
  val iteratee = null
  val netty = null
