...
privileged aspect ActionTraceAspect {
  ...
  before(play.core.server.netty.PlayDefaultUpstreamHandler handler, play.api.mvc.Result result):
    execution(public void play.core.server.netty.PlayDefaultUpstreamHandler+.play$core$server$netty$PlayDefaultUpstreamHandler$$handle$1(play.api.mvc.Result, ..)) &&
    this(handler) &&
    args(result, ..)
  {
  ...
  before(RequestHeader request, Throwable exception):
    execution(public Result play.api.GlobalSettings+.onError(RequestHeader,Throwable)) &&
  ...
  before(RequestHeader request):
    execution(public Result play.api.GlobalSettings+.onHandlerNotFound(RequestHeader)) &&
    args(request)
  {
    ActionTracer tracer = ActionTracer.global();
    if (tracing(tracer)) {
  ...
  before(RequestHeader request, String error):
    execution(public Result play.api.GlobalSettings+.onBadRequest(RequestHeader, String)) &&
    args(request, error)
  {
    ActionTracer tracer = ActionTracer.global();
    if (tracing(tracer)) {
  ...
  before():
    execution(public Result play.api.mvc.Action+.apply(Request))
  {
    ActionTracer tracer = ActionTracer.global();
    if (tracing(tracer)) {
      tracer.action().resultGenerationStart();
